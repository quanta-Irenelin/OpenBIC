...
LOG_MODULE_REGISTER(pm8702, LOG_LEVEL_DBG);
...
uint8_t pm8702_read(uint8_t sensor_num, int *reading)
{
	...
	pm8702_ddr_init_arg *init_arg =
		(pm8702_ddr_init_arg *)sensor_config[sensor_config_index_map[sensor_num]].init_args;
	...
	case chip_temp:
		...
		sval->fraction = 0;
		break;
	case dimm_temp: //dimmB = > 0xc531 to get the all temp
